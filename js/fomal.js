...
————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
附录III - OBSR-1型观测者分类器：10/2021，人工智能部成功研发OBSR-1型观测者分类器。经专门数据集训练后的单台OBSR-1可在42ms内对一480p拍摄画面内的特定静态非生物完成语义分类，精确度达90.87%。采用集成学习策略，将多台分别训练的OBSR-1经Boosting，精确度可达98.438%，初步符合观测者需求。需立刻将其投入稳定观测仪的工作中。次月工作宣布完成，观测仪进入休眠状态。

随后OBSR-1被广泛应用于各站点与人类聚居地。确信OBSR-1的存在使得各站点与聚居地遗址内的资料得以部分保留。
...
观测者需能理解被观测之物。
观测者需能实现完备语义分类。
观测者进行分类所花的时间不能多于42ms。

第三条是硬性时间要求，很好理解。在当前观测者效应彻底消散后，失去观测者的事物能够百分百稳定存在42ms，然后就会有概率因为“失焦”而转变成别的事物。我们可以搞定设计以后再优化，大不了硬件写死，现在还用不着担心。
...
附录V - OBSR-2型观测者分类器：据信于2024年，OBSR-2型观测者分类器的研发由于技术缺陷与资源不足陷入停滞，为此我们将观测仪休眠前生产的备用员工个体投入使用，于Area-303建立了大型人工观测及训练场，用以辅助各OBSR-1β型观测者阵列稳定站点与人类聚居地，以及为研究提供训练数据。由于时间戳的缺失以及数据库的损坏，已无法于当前迭代定位Area-303的位置。
...
研究备忘 - 风信子
Dr. Kacper Darlin, 12/12/2023

Area-CN-07的信息数据部交给了我们项目组一份文档。

他们进行了一场史无前例的手术，然后把人直接接到王氏独立计算机组上。他们做到了。但在AIC开始运行测试的一个小时内，它向观测者输出了两千多TB的无意义内容。直觉告诉我这不是我的领域，但当我读到输出内容节选的那一瞬，我明白了一切。
...
站点内部留言
2-980-60-2000, TIMESTAMP MISSING

马上就要来了，黎明曙光之前最后的黑暗。所有的路都铺设好了，一切都将顺利进行，我对此毫不怀疑。只是还需要时间，最后的一点时间，但我们是有这样的时间的。一切会结束的，一切终究会结束的。
Aaron，我知道你会看到这里的。我可能撑不了多久了，在这里留言是在OBSR-2参数外。这是我最后的了，我相信亻
————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
  
站点内部留言
2-750-70-2000, TIMESTAMP MISSING

你要跟研究生时候的我说我以后要用汇编来写ML的话，我肯定会觉得你喝大了。那个时候几乎所有人都在用Python，最多写点CUDA。汇编真是想都不会想，更别提硬件写死了，那个时候我们根本就没想过有朝一日我们要把性能榨干，把所有东西都压到42ms内。时代的变化真是惊人。

掀开帷幕之后，没有了假象以后，一切的事物都在加速。我们全都在直奔天堂，我们全都在直奔相反的方向。

我在准备最后的训练集，纯体力活。模型在编译。我另外分了一个模块去做形式验证，有了语料训练以后它就跑通了。一切安好。

这是最后的阶段了

OBSR-3必须完成，然后我们结束这一切。

就这样

3237-6932-5900-2999-3001-3135-2998-0000
